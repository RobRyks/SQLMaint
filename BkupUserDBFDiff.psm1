function BkupUserDBFull { [CmdletBinding()]
    param (
            [string]$servername,
            [string]$instance,
            [string]$dbname,
...
#reset error count
$error.clear()
$jobname ='DBA-BkupUserDBDiff-'+$dbname
$step1 = @"
sqlcmd -E -S `$(ESCAPE_SQUOTE(SRVR)) -d master -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = '$dbname', @Directory = N'$backupdir', @BackupType = 'DIFF', @Verify = 'Y', @CleanupTime = 504, @CheckSum = 'Y', @LogToTable = 'Y'" -b
"@
...
if (($instance.Length -eq 0) -or ($backupdir.Length -eq 0) ) {
    write-host 'Error missing required parameters'
    return 1
}

#Create the designated folder
...
$jsch.FrequencyTypes =4      #4 = daily
$jsch.FrequencyInterval = 1   #1 = daily
$jsch.FrequencySubDayTypes =1
$jsch.FrequencySubDayInterval=0
$jsch.FrequencyRelativeIntervals=0
$jsch.FrequencyRecurrenceFactor=0
